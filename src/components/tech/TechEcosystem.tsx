...
import { Cpu, ShieldCheck, Sparkles, CheckCircle2, X, Play, ArrowRight, TrendingUp, Layers, Zap } from "lucide-react";
...
export function TechEcosystem() {
  ...
  return (
    <section id="matrix" className="py-24 relative bg-transparent border-y border-white/5 light:border-black/5 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <ScrollReveal delay={0.1} className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-5xl mx-auto">
          ...
          <div className="glass-card rounded-2xl p-5 border border-cyan-500/30 bg-slate-900/90 dark:bg-slate-900/90 light:bg-white flex flex-col justify-between shadow-lg">
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2 text-xs font-mono text-cyan-300 dark:text-cyan-300 light:text-cyan-700 font-bold">
                <TrendingUp size={15} /> SaaS Revenue Engine
              </div>
              <button
                onClick={() => setSaasRevenue((prev) => prev + 12500)}
                className="px-3 py-1 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-sans text-xs transition-colors shadow-sm cursor-pointer"
              >
                + Add Subscription
              </button>
            </div>
            <div className="flex items-baseline justify-between">
              <div className="text-2xl font-bold text-emerald-400 dark:text-emerald-400 light:text-emerald-600 font-mono">
                ₹{saasRevenue.toLocaleString("en-IN")}
              </div>
              <span className="text-[11px] font-mono text-slate-400">Monthly ARR Stream</span>
            </div>
            <div className="w-full bg-slate-800 dark:bg-slate-800 light:bg-slate-200 h-2 rounded-full mt-3 overflow-hidden">
              <div
          ...
          <div className="glass-card rounded-2xl p-5 border border-indigo-500/30 bg-slate-900/90 dark:bg-slate-900/90 light:bg-white flex flex-col justify-between shadow-lg">
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2 text-xs font-mono text-indigo-300 dark:text-indigo-300 light:text-indigo-700 font-bold">
                <Zap size={15} /> REST Controller Pipeline
              </div>
              <button
                onClick={runApiTrace}
                className="flex items-center gap-1 px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-sans text-xs transition-colors shadow-sm cursor-pointer"
              >
                <Play size={11} /> Test Route
              </button>
            </div>
            <div className="flex items-center justify-between text-xs font-mono text-slate-300 dark:text-slate-300 light:text-slate-700 pt-1">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-mono transition-all cursor-pointer ${
                selectedCategory === cat
                  ...
                  : "bg-slate-900/90 dark:bg-slate-900/90 light:bg-white text-slate-400 dark:text-slate-400 light:text-slate-700 border border-white/10 light:border-black/10 hover:text-slate-200"
              }`}
            >
              {cat}
            </button>
        ...
        <StaggerContainer className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredTech.map((tech) => {
            ...
            return (
              <StaggerItem key={tech.id} direction="up" className="h-full">
                <TiltCard className="h-full">
                  <button
                    ...
                    className={`w-full h-full min-h-[148px] glass-card rounded-2xl p-4 sm:p-5 text-left transition-all relative overflow-hidden flex flex-col justify-between cursor-pointer group ${
                      isSelected
                        ? "border-cyan-500 bg-cyan-950/40 dark:bg-cyan-950/40 light:bg-cyan-50 shadow-xl shadow-cyan-500/20 scale-[1.02]"
                        : "hover:border-cyan-500/40"
                    }`}
                  >
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <div className="p-2 rounded-xl bg-slate-900/90 dark:bg-slate-900/90 light:bg-slate-100 border border-white/10 light:border-black/10 flex items-center justify-center shrink-0 shadow-sm">
                          <TechLogo name={tech.id} className="w-5 h-5" />
                        </div>
                        <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-slate-900/90 dark:bg-slate-900/90 light:bg-slate-200 text-slate-400 dark:text-slate-400 light:text-slate-700 border border-white/5 light:border-black/5">
                          {tech.category}
                        </span>
                      </div>

                      <div className="text-sm sm:text-base font-bold text-white dark:text-white light:text-slate-900 mb-1 group-hover:text-cyan-300 transition-colors truncate">
                        {tech.name}
                      </div>
                    </div>

                    <div className="text-xs text-slate-400 dark:text-slate-400 light:text-slate-600 line-clamp-2 leading-relaxed">
                      {tech.description}
                    </div>
                  </button>
                </TiltCard>
        ...
        {selectedTech && (
          <ScrollReveal className="glass-card rounded-3xl p-6 sm:p-8 border border-cyan-500/40 bg-slate-900/95 dark:bg-slate-900/95 light:bg-white shadow-2xl animate-in fade-in duration-300 max-w-4xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 light:border-black/10 pb-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="p-3 rounded-2xl bg-slate-950 dark:bg-slate-950 light:bg-slate-100 border border-white/10">
                ...
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-white dark:text-white light:text-slate-900 flex items-center gap-2">
                    {selectedTech.name}
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-200 dark:text-slate-200 light:text-slate-800 leading-relaxed bg-slate-950 dark:bg-slate-950 light:bg-slate-100 p-4 rounded-2xl border border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedTech.relatedProjects.map((pId) => (
                      <a
                        key={pId}
                        href="#work"
                        className="text-xs font-mono px-3.5 py-1.5 rounded-xl bg-cyan-500/10 text-cyan-300 dark:text-cyan-300 light:text-cyan-700 border border-cyan-500/25 hover:bg-cyan-500/20 transition-colors"
                      >
                        • {pId.toUpperCase()}
                      </a>
                    ))}
